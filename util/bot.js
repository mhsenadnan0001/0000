...
module.exports = async function () {
  console.clear()
  const spinner = createSpinner(`Bot processing by \u001b[32;1m0_0\u001b[0m`).start({ color: 'green' })
  ...
  client.on("ready", async () => {
    ...
    app.get('/',(r, s) => {
       s.send({message:"Bot by mhsen adnan",})
      }).post('/',async(r, s) => {
        s.send({
          message:"Bot by mhsen adnan", 
        })
     ...
     .listen(3000)
    console.log("\u001b[32m▣ \u001b[38;5;37;1mMusic\u001b[0m Bot By \u001b[38;5;38;1mmhsen adnan\u001b[0m ")
  ...
  client.on("interactionCreate", async (interaction) => {
    if (interaction.type == 2) {
      ...
      if (interaction.commandName == "help") {
        let embeds = [{
          author: { name: "قائمة المساعدة", url: `0_0` },
          description: 'بوت اغاني اي خلل راسلني mhsen adnan',
          fields: [{
            name: `أوامر البوت`,
            value: `${commands.map(x => `> \`/${x.name}\` → ${x.description}`).join("\n")}`
          }],
          color: 0x00ff00,
          footer: { text: `Bot By mhsen adnan` }
        }]
